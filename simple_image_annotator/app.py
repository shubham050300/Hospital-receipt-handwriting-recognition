...
def main():
    ...
    print(not_end)
    return render_template('main.html', not_end=not_end, directory=directory, image=image, labels=labels, head=app.config["HEAD"] + 1, len=len(app.config["FILES"]))
...
@app.route("/bye")
def bye():
    return send_file("thatsit.png", mimetype='image/gif')
